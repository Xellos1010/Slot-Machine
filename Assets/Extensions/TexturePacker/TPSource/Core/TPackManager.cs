...
public class TPackManager {
	...
	public static void load(string atlas) {
		try {
			...
			load(atlas_data.text, atlas_texture, atlas);
	...
	public static void load(string atlasData, Texture atlasTexture, string AtlasName) {
		try {
			
			if(atlases.ContainsKey(atlasData)) {
				return;
			}
			
			atlases.Add(AtlasName, new TPAtlas(atlasData, atlasTexture, AtlasName));
		} catch(System.Exception ex) {
			Debug.LogError("Atlas Can not be loaded form: " + AtlasName);
			Debug.LogError(ex.Message);
		}
